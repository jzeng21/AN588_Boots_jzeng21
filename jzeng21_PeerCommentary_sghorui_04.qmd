...
KC <- KC %>% # nice mutate usage here I didn't use mutate but it worked for you!I directly assigned using data when trying to run the code i did get an error but i am not sure if it is just specific to me and not when you run it. - jason
  mutate(
    logHR = log(HomeRange_km2),
    logBM = log(Body_mass_female_mean) # Creating new variables with log transformations
  ) %>% # nice removal of non numerical values by using is na - jason but 
...
```{r}
#Here overall i did the same as you
...
```{r}
#overall the code here is similar to mine. I did both setting the boot then setting the seed. - jason 
...
#after running the results I would like to see your interpretation of the results and what the results mean - jason
```
...
#overall very similar good job on this-jason 
```
...
#nice plot - jason 
# I think overall there does not seem to be much issue although when I tried to run the first code it had an error but I am not sure if that is just specific to me- jason
```
